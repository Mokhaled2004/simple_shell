...
/**
 * _exitt - Handle the exit command with the provided exit status
 * @args: The arguments for the exit command
 * @exit_status: Reference to the exit status variable
 */
void _exitt(char *args[], int *exit_status)
{
if (args[1] != NULL)
{
 ...
 * @exit_status: Reference to the exit status variable
 */
void execute_command(char __attribute__((unused)) *command,
...
while (_printf("$ "), (read = getline(&line, &line_length, stdin)) != -1)
{
i = 0;
args[i] = strtok(line, " \n");
while (args[i])
